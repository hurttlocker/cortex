...
func TestLoadTelemetry_ParsesJSONLAndSkipsMalformed(t *testing.T) {
	...
	out := RenderReport(path, BuildReport(events, skipped), nil, cfg)
...
func TestExecuteWithOptions_StrictModeNonZeroExit(t *testing.T) {
	tmp := t.TempDir()
	path := filepath.Join(tmp, "reason-telemetry.jsonl")
	content := strings.Join([]string{
		`{"mode":"one-shot","provider":"openrouter","model":"openai-codex/gpt-5.2","wall_ms":25000,"cost_known":true,"cost_usd":0.0012}`,
	...
	if res.ExitCode != 2 {
		t.Fatalf("expected strict mode exit code 2, got %d", res.ExitCode)
	}
	if len(res.Warnings) == 0 {
		t.Fatal("expected warnings in strict mode")
	}
}
...
func TestParseArgs_FlagParsing(t *testing.T) {
	...
	if opts.FilePath != "/tmp/x.jsonl" {
		t.Fatalf("unexpected file path: %s", opts.FilePath)
	}
	if opts.Guardrails.OneShotP95WarnMS != 15000 {
		t.Fatalf("unexpected p95 threshold: %d", opts.Guardrails.OneShotP95WarnMS)
	}
	if opts.Guardrails.RecursiveKnownCostMinShare != 0.9 {
		t.Fatalf("unexpected completeness threshold: %.2f", opts.Guardrails.RecursiveKnownCostMinShare)
	}
	if opts.Guardrails.WarnOnly {
		t.Fatalf("expected warn-only false")
	}
}
